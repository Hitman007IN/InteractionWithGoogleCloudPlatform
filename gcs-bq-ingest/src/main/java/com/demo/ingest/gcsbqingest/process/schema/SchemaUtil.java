...
package com.demo.ingest.gcsbqingest.process.schema;

import com.google.api.services.bigquery.model.TableFieldSchema;
import com.google.api.services.bigquery.model.TableSchema;
import com.demo.ingest.gcsbqingest.util.PrettyPrinter;
...
public class SchemaUtil {
	 ...
	 *
	 * @param input
	 *            the values to infer schema from
	...
	public static TableSchema generateBigQueryTableSchema(String[] headers, FieldType[] types) {
		...
		if (headers.length != types.length) {
			throw new IllegalArgumentException(String.format("Encountered invalid input:\nheaders: %s\ntypes: %s",
					PrettyPrinter.print(Arrays.asList(headers)), PrettyPrinter.print(Arrays.asList(types))));
		}
	...
	/**
	 * Merges a series of AVRO schema. The rule is simple: we choose the most
	 * generic type, e.g. we choose string between int and string.
	 *
	 * @param types
	 *            a list of schema to merge
	...
	public static List<FieldType> merge(List<FieldType> s, List<FieldType> t) {
		...
		Preconditions.checkArgument(s.size() == t.size(), "Number of fields in both schema should match.");
		return IntStream.range(0, s.size()).mapToObj(i -> FieldType.getCommonType(s.get(i), t.get(i)))
	...
	/**
	 * Boolean is different because {@link Boolean#parseBoolean(String)} always
	 * return true or false.
	 */
	private static boolean convertToBoolean(String value) {
		return "True".equalsIgnoreCase(value) || "False".equalsIgnoreCase(value) || "Yes".equalsIgnoreCase(value)
				|| "No".equalsIgnoreCase(value) || "Y".equalsIgnoreCase(value) || "N".equalsIgnoreCase(value)
	...
	public static String convertToDate(String value) {
		try {
			return LocalDate.parse(value, DateTimeFormatter.ISO_DATE).format(DateTimeFormatter.ISO_LOCAL_DATE);
	...
	public static String convertToTime(String value) {
		try {
			return LocalTime.parse(value, DateTimeFormatter.ISO_TIME).format(DateTimeFormatter.ISO_LOCAL_TIME);
		} catch (DateTimeParseException e) {
			return null;
		}
	}
