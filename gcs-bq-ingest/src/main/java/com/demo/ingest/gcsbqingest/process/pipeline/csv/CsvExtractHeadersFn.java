...
/**
 * A {@link DoFn} that extracts the first line of a {@link ReadableFile} as
 * headers and apply simple validations.
 */
public class CsvExtractHeadersFn extends DoFn<ReadableFile, KV<String, String[]>> {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(Channels.newInputStream(file.open())))) {
			...
			if (!validateHeaders(headers)) {
				throw new IllegalArgumentException(
						String.format("%s of file %s contains invalid headers. Headers are required for every file.",
	...
	/**
	 * Normalizes headers by removing quotes (only those at the beginning and at the
	 * end), and replacing any non-word characters with underscores to meet the
	 * requirements on column names from both AVRO and BigQuery.
	 *
	 ...
	 *
	 * 1. There are no duplicate values 2. There are no non-string values 3. There
	 * are no empty values
	 *
	 * @return false if we know for sure that these are not valid headers, true
	 *         meaning based on our heuristics these look like valid headers.
	 */
