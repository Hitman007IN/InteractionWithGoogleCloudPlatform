...
/**
 * Parses a chunk of data with the regular expressions. Only the default
 * encoding on JVM (UTF-8) is supported.
 */
public class CsvParseDataAdvanceFn extends DoFn<KV<String, byte[]>, KV<String, String[]>> {
	...
	public void parse(ProcessContext ctx) {
		...
		Arrays.stream(records).map(fieldSplitPattern::split).forEach(fields -> ctx.output(KV.of(name, fields)));
	}
}
