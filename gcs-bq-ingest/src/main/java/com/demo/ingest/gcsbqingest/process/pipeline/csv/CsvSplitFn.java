...
public abstract class CsvSplitFn extends DoFn<ReadableFile, KV<String, Set<Long>>> {
	...
	public CsvSplitFn(CsvConfiguration config, PCollectionView<Map<String, String[]>> headersView) {
		this.config = config;
	...
	public void split(ProcessContext ctx) throws IOException {
		...
		for (int i = 0; i < splitPoints; i++) {
			Long splitPoint = calcSplitPoint(ch, startSplitCheckPosition(i),
	...
	// Calculates the split point in the range specified as parameters (start
	// inclusive, end
	// exclusive). If no valid split point is found, null will be returned.
	@Nullable
	protected abstract Long calcSplitPoint(SeekableByteChannel ch, long start, long end) throws IOException;

	/**
	 * Returns the position of the first byte on second row, since we require first
	 * row to be headers.
	 */
